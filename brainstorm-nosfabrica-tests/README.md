# Brainstorm NosFabrica Tests

Automated test suite for the NosFabrica Brainstorm pipeline.
...
├── KEYS.md                ← Test account registry (no secrets — just env var names + pubkeys)
├── propagation/           ← Kind 3 follow propagation test (single event)
│   ├── run-test.sh
│   ├── README.md          ← (TODO: move propagation-specific docs here)
│   └── results/           ← JSON results per run
├── stress-test-1/         ← Burst follow propagation (10 events in 60s)
│   ├── setup.sh           ← One-time: creates 100 accounts + coordinator
│   ├── run-test.sh        ← Repeatable: burst publish + monitor (nonce rotation)
│   ├── reset.sh           ← Clear sender follow lists + reset nonce
│   ├── state.json         ← Nonce counter (auto-managed)
│   ├── README.md
│   ├── accounts.json      ← Generated by setup (pubkeys + metadata)
│   ├── coordinator.json   ← Generated by setup
│   └── results/           ← JSON results per run
├── new-customer/          ← End-to-end new customer onboarding test
│   ├── run-test.sh        ← Fresh identity → signup → kind 10040 → monitor TAs
│   ├── state.json         ← Run counter
│   ├── README.md
│   └── results/           ← JSON results per run
└── <future-test>/         ← Each test type gets its own subdirectory
    ├── run-test.sh
    ├── README.md
    └── results/
```
...
See `KEYS.md` for how test account nsecs are stored and discovered by agents.
**No secrets are stored in this repository.** Keys live at `~/.config/nosfabrica-tests/keys.env`.

---

## Propagation Test

Smoke test to verify that nostr follow events propagate correctly
...
- `jq`
- `curl`
- Nous's nsec accessible via **one** of (checked in this order):
  1. `NOSTR_SECRET_KEY` environment variable (nsec or hex)
  2. `~/.config/nosfabrica-tests/keys.env` — sources `NOUS_NSEC` (see `KEYS.md`)
  3. 1Password desktop app unlocked — script reads from:
     `op read "op://Personal/Nous - Nostr Key/password"`

## Usage

```bash
cd brainstorm-nosfabrica-tests/propagation
./run-test.sh
```

Or with explicit key:
```bash
NOSTR_SECRET_KEY=nsec1... ./propagation/run-test.sh
```
